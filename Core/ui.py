import re
import threading
import customtkinter as ctk

ctk.set_appearance_mode("dark")
ctk.set_default_color_theme("blue")

# Matches summary lines like: [INFO] Label1 X | Label2 Y | Label3 Z
_SUMMARY_RE = re.compile(r'^\[INFO\]\s*.+\d(?:\s\|\s.+\d)+$')


# ── Progress wrapper ──────────────────────────────────────────────────────────

class ProgressWrapper:
    """
    Translates ttk-style progress["value"] / progress["maximum"] assignments
    into CTkProgressBar.set() calls so downloader.py needs no changes.

    Supports:
      - pulse() / stop_pulse()  — indeterminate animation
      - set_status(text)        — updates the status label below the bar
    """
    def __init__(self, bar: ctk.CTkProgressBar, root: ctk.CTk,
                 label: ctk.CTkLabel | None = None):
        self._bar     = bar
        self._root    = root
        self._label   = label
        self._value   = 0
        self._maximum = 100
        self._pulse_stop: threading.Event | None = None

    def __setitem__(self, key, val):
        if key == "value":
            self._value = int(val)
            if self._pulse_stop is None:
                frac = self._value / self._maximum if self._maximum else 0
                self._root.after(0, lambda f=frac: self._bar.set(min(max(f, 0.0), 1.0)))
        elif key == "maximum":
            self._maximum = max(int(val), 1)

    def __getitem__(self, key):
        return self._value if key == "value" else self._maximum

    def set_status(self, text: str):
        """Update the status label (thread-safe)."""
        if self._label:
            self._root.after(0, lambda t=text: self._label.configure(text=t))

    def pulse(self):
        """Start bouncing animation."""
        if self._pulse_stop is not None:
            return
        stop_evt = threading.Event()
        self._pulse_stop = stop_evt
        self.set_status("Running…")

        def _animate():
            pos, direction = 0.0, 1
            while not stop_evt.is_set():
                self._root.after(0, lambda p=pos: self._bar.set(p))
                stop_evt.wait(0.025)
                pos += 0.025 * direction
                if pos >= 1.0:
                    pos, direction = 1.0, -1
                elif pos <= 0.0:
                    pos, direction = 0.0, 1

        threading.Thread(target=_animate, daemon=True).start()

    def stop_pulse(self):
        """Stop animation and restore bar."""
        if self._pulse_stop is None:
            return
        self._pulse_stop.set()
        self._pulse_stop = None
        frac = self._value / self._maximum if self._maximum else 0
        self._root.after(0, lambda: self._bar.set(min(max(frac, 0.0), 1.0)))
        self.set_status("Idle")

    def pack(self, **kw):  self._bar.pack(**kw)
    def grid(self, **kw):  self._bar.grid(**kw)
    def place(self, **kw): self._bar.place(**kw)


# ── Output area ───────────────────────────────────────────────────────────────

def build_output_area(root: ctk.CTk) -> ctk.CTkTextbox:
    """
    Builds the main log output textbox.
    No config block — config lives in the sidebar.
    Starts with just the 'Operational Log:' header.
    """
    textbox = ctk.CTkTextbox(
        root,
        height=396,
        width=950,
        font=("Consolas", 15),
        fg_color="#171717",
        text_color="#d0d0d0",
        scrollbar_button_color="#2e2e2e",
        scrollbar_button_hover_color="#444444",
        border_color="#2a2a2a",
        border_width=1,
        wrap="word",
        state="disabled",
    )
    textbox.pack(fill="both", expand=True, padx=16, pady=(4, 4))

    inner = textbox._textbox
    inner.tag_configure("bold",     font=("Consolas", 15, "bold"))
    inner.tag_configure("logtitle", font=("Consolas", 15, "bold"), foreground="#8ab4f8")
    textbox.tag_config("dotted",    foreground="#383838")
    textbox.tag_config("ok",        foreground="#81c995")
    textbox.tag_config("warn",      foreground="#f9c74f")
    textbox.tag_config("err",       foreground="#f28b82")

    return textbox


def make_print_output(root: ctk.CTk, textbox: ctk.CTkTextbox):
    """Return a thread-safe print_output(msg) with colour-coded prefixes."""
    def print_output(msg: str):
        def _append():
            textbox.configure(state="normal")

            leading = len(msg) - len(msg.lstrip('\n'))
            if leading:
                textbox.insert("end", "\n" * leading)

            stripped = msg.lstrip()

            if _SUMMARY_RE.match(stripped):
                textbox.insert("end", "[INFO] ", "bold")
                body = stripped.split("] ", 1)[1] if "] " in stripped else stripped[7:]
                parts = [p.strip() for p in body.split("|")]
                for i, part in enumerate(parts):
                    label, value = (part.rsplit(" ", 1) if " " in part else (part, ""))
                    textbox.insert("end", f"{label}: ", "bold")
                    textbox.insert("end", value)
                    if i < len(parts) - 1:
                        textbox.insert("end", " | ")
                textbox.insert("end", "\n")
            elif stripped.startswith(("[OK]", "[MOVED]")):
                textbox.insert("end", msg + "\n", "ok")
            elif stripped.startswith("[SKIP]"):
                textbox.insert("end", msg + "\n", "warn")
            elif stripped.startswith(("[ERROR]", "[WARN]")):
                textbox.insert("end", msg + "\n", "err")
            else:
                textbox.insert("end", msg + "\n")

            textbox.see("end")
            textbox.configure(state="disabled")

        root.after(0, _append)
    return print_output


def init_output_log(textbox: ctk.CTkTextbox):
    """Write the initial 'Operational Log:' header into the textbox."""
    textbox.configure(state="normal")
    textbox.delete("1.0", "end")
    textbox.insert("end", "Waiting for action…\n")
    textbox.configure(state="disabled")


# ── Progress bar ──────────────────────────────────────────────────────────────

def build_progressbar(parent) -> ProgressWrapper:
    """
    Creates a centred progress bar + status label.
    Returns a ProgressWrapper.
    """
    frame = ctk.CTkFrame(parent, fg_color="transparent")
    frame.pack(pady=(4, 4))

    bar = ctk.CTkProgressBar(
        frame,
        width=600,
        height=5,
        fg_color="#252525",
        progress_color="#4a90d9",
        corner_radius=3,
    )
    bar.set(0)
    bar.pack()

    status_label = ctk.CTkLabel(
        frame,
        text="Idle",
        font=ctk.CTkFont("Segoe UI", 11),
        text_color="#8ab4f8",
    )
    status_label.pack(pady=(2, 0))

    return ProgressWrapper(bar, parent, label=status_label)


# ── Last run summary bar ──────────────────────────────────────────────────────

def build_last_run_bar(parent) -> ctk.CTkLabel:
    """
    A small labelled bar below the progress area that shows the last task result.
    Call update_last_run(label, task_name, ok, skipped, errors) after each task.
    """
    frame = ctk.CTkFrame(
        parent,
        fg_color="#171717",
        border_color="#2a2a2a",
        border_width=1,
        corner_radius=6,
        height=24,
    )
    frame.pack(fill="x", padx=16, pady=(0, 4))

    lbl = ctk.CTkLabel(
        frame,
        text="Last Run Summary",
        font=ctk.CTkFont("Consolas", 11),
        text_color="#888888",
    )
    lbl.pack(pady=4)
    return lbl


def update_last_run(label: ctk.CTkLabel, task_name: str,
                    ok: int, skipped: int, errors: int):
    """Update the last-run bar after a task completes (thread-safe)."""
    import datetime
    now = datetime.datetime.now().strftime("%I:%M %p")
    parts = [f"Last run: {task_name}", f"{ok} ok"]
    if skipped:
        parts.append(f"{skipped} skipped")
    if errors:
        parts.append(f"{errors} errors")
    parts.append(now)
    label._root.after(0, lambda: label.configure(text="  ·  ".join(parts)))


# ── Toast notification ────────────────────────────────────────────────────────

class ToastManager:
    """
    Shows a small floating toast window near the bottom of the screen.
    Call show(message, level) where level is 'ok', 'warn', or 'err'.
    """
    _COLORS = {
        "ok":   "#81c995",
        "warn": "#f9c74f",
        "err":  "#f28b82",
        "info": "#8ab4f8",
    }

    def __init__(self, root: ctk.CTk):
        self._root = root
        self._win: ctk.CTkToplevel | None = None
        self._hide_job = None

    def show(self, msg: str, level: str = "ok"):
        self._root.after(0, lambda: self._show(msg, level))

    def _show(self, msg: str, level: str):
        if self._hide_job:
            self._root.after_cancel(self._hide_job)
            self._hide_job = None
        if self._win:
            try:
                self._win.destroy()
            except Exception:
                pass

        color = self._COLORS.get(level, "#8ab4f8")
        win = ctk.CTkToplevel(self._root)
        win.overrideredirect(True)
        win.attributes("-topmost", True)
        win.configure(fg_color="#1e1e1e")

        ctk.CTkLabel(
            win,
            text=f"  {msg}  ",
            font=ctk.CTkFont("Segoe UI", 13),
            text_color=color,
            fg_color="#1e1e1e",
        ).pack(padx=2, pady=8)

        win.update_idletasks()
        w = win.winfo_width()
        h = win.winfo_height()
        sw = self._root.winfo_screenwidth()
        sh = self._root.winfo_screenheight()
        win.geometry(f"{w}x{h}+{(sw - w) // 2}+{sh - h - 80}")

        self._win = win
        self._hide_job = self._root.after(3500, self._hide)

    def _hide(self):
        if self._win:
            try:
                self._win.destroy()
            except Exception:
                pass
            self._win = None
        self._hide_job = None


# ── Config sidebar ────────────────────────────────────────────────────────────

def _truncate(value: str, maxlen: int = 36) -> str:
    if not value or value == "[Not Set]":
        return "[Not Set]"
    return ("…" + value[-(maxlen - 1):]) if len(value) > maxlen else value


def build_config_sidebar(parent, config: dict, downloader_msg: str):
    """
    Styled config sidebar. Pack it yourself.
    Returns (frame, refresh_fn).

    [Not Set] values render in amber as a warning.
    """
    MISSING_COLOR = "#f9c74f"
    KEY_COLOR     = "#aaaaaa"
    VAL_COLOR     = "#c0c0c0"
    OK_COLOR      = "#81c995"
    ERR_COLOR     = "#f28b82"

    frame = ctk.CTkFrame(
        parent,
        fg_color="#171717",
        border_color="#2a2a2a",
        border_width=1,
        corner_radius=6,
    )

    # Header
    ctk.CTkLabel(
        frame,
        text="Config",
        font=ctk.CTkFont("Consolas", 13, weight="bold"),
        text_color="#8ab4f8",
    ).pack(anchor="w", padx=12, pady=(6, 2))

    ctk.CTkFrame(frame, height=1, fg_color="#2a2a2a").pack(fill="x")

    rows_frame = ctk.CTkFrame(frame, fg_color="transparent")
    rows_frame.pack(fill="both", expand=True, padx=10, pady=(6, 8))

    def _section(title: str):
        ctk.CTkLabel(
            rows_frame,
            text=title,
            font=ctk.CTkFont("Consolas", 10),
            text_color="#666666",
        ).pack(anchor="w", pady=(4, 1))

    def _divider():
        ctk.CTkFrame(rows_frame, height=1, fg_color="#2e2e2e").pack(fill="x", pady=4)

    def _row(key: str, val: str):
        missing = (val == "[Not Set]")
        r = ctk.CTkFrame(rows_frame, fg_color="transparent")
        r.pack(fill="x", pady=1)
        ctk.CTkLabel(
            r,
            text=f"· {key}",
            font=ctk.CTkFont("Consolas", 11),
            text_color=KEY_COLOR,
            width=112,
            anchor="w",
        ).pack(side="left")
        ctk.CTkLabel(
            r,
            text=val,
            font=ctk.CTkFont("Consolas", 11),
            text_color=MISSING_COLOR if missing else VAL_COLOR,
            anchor="w",
            wraplength=300,
            justify="left",
        ).pack(side="left", fill="x", expand=True)

    def refresh():
        for w in rows_frame.winfo_children():
            w.destroy()

        lib = _truncate(config.get("library_folder", "[Not Set]"))
        stg = _truncate(config.get("staging_folder", "[Not Set]"))
        csv = _truncate(config.get("csv_path",        "[Not Set]"))

        _section("PATHS")
        _row("Library",    lib)
        _row("Staging",    stg)
        _row("Shazam CSV", csv)

        _divider()
        _section("TAGS")

        song_tags = config.get("song_tags") or []
        web_tags  = config.get("web_tags")  or []
        song_disp = " | ".join(song_tags) if song_tags else "[Not Set]"
        web_disp  = " | ".join(web_tags)  if web_tags  else "[Not Set]"
        _row("Song Tags", song_disp)
        _row("Web Tags",  web_disp)

        _divider()
        _section("STATUS")
        _row("Last Scanned", config.get("last_scanned_date", "[Not Set]"))

        dl_ok    = "Ready" in downloader_msg
        dl_color = OK_COLOR if dl_ok else ERR_COLOR
        r = ctk.CTkFrame(rows_frame, fg_color="transparent")
        r.pack(fill="x", pady=1)
        ctk.CTkLabel(r, text="· Downloader", font=ctk.CTkFont("Consolas", 11),
                     text_color=KEY_COLOR, width=112, anchor="w").pack(side="left")
        ctk.CTkLabel(r, text=_truncate(downloader_msg, 28),
                     font=ctk.CTkFont("Consolas", 11),
                     text_color=dl_color, anchor="w").pack(side="left")

    refresh()
    return frame, refresh


# ── Utilities ─────────────────────────────────────────────────────────────────

def center_window(win: ctk.CTk):
    win.update_idletasks()
    w, h = win.winfo_width(), win.winfo_height()
    sw, sh = win.winfo_screenwidth(), win.winfo_screenheight()
    win.geometry(f"{w}x{h}+{(sw - w) // 2}+{(sh - h) // 2}")


def run_in_thread(fn, *args, **kwargs):
    threading.Thread(target=lambda: fn(*args, **kwargs), daemon=True).start()
